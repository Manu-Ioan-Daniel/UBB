...
MyVector testCopyIterate(MyVector v) {
	int totalPrice = 0;
	for (const auto& el : v) {
		totalPrice += el.getNrOre();
	}
	Disciplina d{ "total",totalPrice,"tt","tt" };
	v.push_back(d);
	return v;
}
...
void addDisciplina(MyVector& v, const int cate) {
	for (int i = 0; i<cate; i++) {
		Disciplina d{ std::to_string(i) + "_denumire",i + 10,std::to_string(i)+"_type",std::to_string(i) + "_cadru" };
		v.push_back(d);
	}
}
