...
public class StageManager {
     ...
     * @param service serviciul care va fi folosit de controllerul ferestrei de logare
     */
    public void openLoginWindow(Service service) {
        Tuple<Scene, LoginController> tuple = FXMLUtils.load("/view/loginView.fxml");
        tuple.getSecond().setService(service);
        tuple.getSecond().setStageManager(this);

        Stage stage = new Stage();
        stage.setScene(tuple.getFirst());
        stage.setTitle("Login");
        stage.show();
        openStages.put("login", stage);
    }

    /***
     * Inchide fereastra de logare
     */
    public void closeLoginWindow() {
        Stage login = openStages.remove("login");
        if (login != null) {
            login.close();
        }
    }
