...
sequenceDiagram
    ...
    A->>B: 7: clickApprove(dealerId)
    B->>C: 8: approveDealer(dealerId)
    C->>S: 9: updateDealerStatus(dealerId, "APPROVED")
    C->>S: 10: assignRole(dealerId, "DEALER")
    S-->>C: 11: dealerActivated
    C-->>B: 12: approvalSuccess
    destroy C
    C-->>C: 13: <<destroy>>
    B-->>A: 14: showUpdatedList()
```
...
sequenceDiagram
    ...
    B->>C: 8: suspendAccount(userId)
    C->>S: 9: updateStatus(userId, "SUSPENDED")
    C->>S: 10: terminateActiveSessions(userId)
    S-->>C: 11: accountSuspended
    C-->>B: 12: suspensionSuccess
    destroy C
    C-->>C: 13: <<destroy>>
    B-->>A: 14: showUpdatedUserList()
```
